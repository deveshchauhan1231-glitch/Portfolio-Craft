export async function deployToNetlify(zipBuffer, ownerName = 'portfolio', existingSiteId = null) {
  ...
  const site = existingSiteId ? { id: existingSiteId } : await createSite(ownerName);
  ...
  return {
    siteId,
    siteName: site.name || makeSiteName(ownerName),
    deployId: deploy.id,
...
function makeSiteName(ownerName) {
  const slug = String(ownerName || 'portfolio')
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 30) || 'portfolio';

  return `${slug}-portfolioCart-${Date.now().toString(36)}`;
}
