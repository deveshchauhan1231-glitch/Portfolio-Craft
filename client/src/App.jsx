...
export default function App() {
  ...
  async function getDesignSuggestions() {
    setStatus('Applying design suggestions...');
    const result = await postJson('/api/ai/design-suggestions', { theme, data });
    if (result.colors) setTheme((current) => ({ ...current, ...result.colors }));
    setStatus(result.layoutTips?.join(' ') || 'Theme suggestions applied.');
  }
  ...
  async function deploySite() {
    setStatus('Creating a fresh Netlify site...');
    const result = await postJson('/api/deploy/netlify', { data, theme, inputs });
    setDeployUrl(result.url);
    setStatus(`Deployment finished as ${result.siteName || 'new Netlify site'}.`);
  }
    ...
    <div className="shell">
      ...
      {page === 'editor' && (
        <EditorPage
          ...
          onDesign={() => handleAsync(getDesignSuggestions, null, setStatus)}
...
function PricingPage() {
  return (
    <main className="contentPage">
      <div className="pageHead">
        <p>Pricing</p>
        <h1>Hackathon-friendly by design.</h1>
      </div>
      <div className="pricingCard">
        <BadgeCheck size={28} />
        <h2>Local Builder</h2>
        <p>Use your own Groq and Netlify API keys. Export static sites freely and deploy as many fresh Netlify sites as your account allows.</p>
        <ul>
          <li>PDF resume parsing</li>
          <li>Manual correction UI</li>
          <li>AI bio and design helpers</li>
          <li>Static zip and Netlify publish</li>
        </ul>
      </div>
    </main>
  );
...
function EditorPage(props) {
  const {
    ...
    previewHtml, onUpload, onBio, onDesign, onDownload, onDeploy, uploadHeroImage, updateData
  } = props;
    ...
    <div className="editorApp">
      ...
      <aside className="controlPanel">
        ...
        <div className="buttonRow">
          ...
          <IconButton icon={<Palette size={17} />} label="Design" onClick={onDesign} />
        </div>
...
focus: ${(data.skills || []).slice(0, 4).map(escapeHtml).join(', ') || 'Add skills'}</pre></div>`;
  ...
  return `<!doctype html><html><head><style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--text);font-family:${theme.font};line-height:1.55}
    ...
    .project{display:grid;grid-template-columns:1fr 180px;gap:18px}.visual{border-radius:10px;background:linear-gradient(135deg,var(--primary),var(--secondary));display:grid;place-items:center;color:white;font-size:42px;font-weight:900}
